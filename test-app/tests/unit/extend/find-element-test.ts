...
module(`Extend | findElement`, function (hooks) {
  ...
  test('resetScope param', async function (assert) {
    ...
    assert.deepEqual(
      findElement(page, '.lorem', { resetScope: true })
        .toArray()
        .map((el) => el.innerText),
      ['1']
    );
  ...
  test('contains param', async function (assert) {
    ...
    assert.deepEqual(
      findElement(page, '.lorem', { contains: 'Word' })
        .toArray()
        .map((el) => el.id),
      ['2']
    );
  ...
  test('scope param', async function (assert) {
    ...
    assert.deepEqual(
      findElement(page, '.lorem', { scope: '.ipsum' })
        .toArray()
        .map((el) => el.innerText),
      ['2']
    );
  ...
  test('visible param', async function (assert) {
    ...
    assert.deepEqual(
      findElement(page, '.lorem', { visible: true })
        .toArray()
        .map((el) => el.innerText),
      ['2']
    );
  ...
  test('last param', async function (assert) {
    ...
    assert.deepEqual(
      findElement(page, '.lorem', { last: true })
        .toArray()
        .map((el) => el.innerText),
      ['3']
    );
  ...
  test('multiple param', async function (assert) {
    ...
    assert.deepEqual(
      findElement(page, '.lorem', { multiple: true })
        .toArray()
        .map((el) => el.innerText),
      ['1', '2', '3']
    );
